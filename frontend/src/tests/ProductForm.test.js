...
// Polyfill requestSubmit cho JSDOM (nếu chưa có trong setupTests.js)
...
describe('ProductForm Component Tests', () => {
  ...
  it('TC2: Nhap du lieu name & price', () => {
    ...
    // Input type number trả về string
  ...
  it('TC3: Submit hop le goi onSave voi payload', async () => {
    ...
    // Kiểm tra object được gửi đi (khớp với logic trong ProductForm)
  ...
  it('TC4: Submit rong hien thi loi va khong goi onSave', () => {
    ...
    // Text lỗi khớp với productValidation.js
  ...
  it('TC5: Loi dong thoi name & price', async () => {
    ...
    fireEvent.change(nameEl, { target: { value: 'AB' } }); // < 3 ký tự
    fireEvent.change(priceEl, { target: { value: '-1000' } }); // Số âm
    
    // Blur để trigger validation UI (nếu code dùng onBlur)
  ...
  it('TC7: Category missing hien thi loi category', async () => {
    // Lưu ý: Trong ProductForm logic mới, category thường có default value.
    // Test case này giả định ta set value rỗng để test validation.
    renderCreate();
    ...
    // Logic trong productValidation.js không bắt lỗi category rỗng nếu input không nằm trong whitelist,
    // hoặc trả về "Danh muc san pham khong hop le"
    // Tùy logic bạn implement, ở đây check lỗi xuất hiện là được.
    // Nếu bạn comment lại logic category trong validation.js thì test này có thể cần điều chỉnh.
    /* Nếu validation.js có check: if (!category) errors.category = '...'
